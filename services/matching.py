...
def match_welfare_programs(
    ...
    print(f"🔍 매칭 디버깅 시작")
    print(f"{'='*60}")
    print(f"0. 전체 프로그램 수: {len(matched)}개")
    
    # 이전에 추천한 프로그램 제외
    if exclude_programs:
        matched = matched[~matched['program_name'].isin(exclude_programs)]
        print(f"1. 이전 추천 제외 후: {len(matched)}개 (제외: {len(exclude_programs)}개)")
    ...
    if include_category:
        ...
        print(f"2. 카테고리 '{include_category}' 필터링 후: {len(matched)}개 (제외: {before_count - len(matched)}개)")
    ...
    if user_info.get('age'):
        ...
        print(f"3. 나이({age}세) 필터링 후: {len(matched)}개 (제외: {before_count - len(matched)}개)")
    ...
    print(f"4. 특수조건 필터링 후: {len(matched)}개 (제외: {before_count - len(matched)}개)")
    ...
    if not is_newlywed:
        ...
        print(f"5. 신혼부부 프로그램 제외 후: {len(matched)}개 (제외: {before_count - len(matched)}개)")
    ...
    if user_info.get('residence'):
        ...
        if residence and not is_seoul:
            ...
            print(f"6. 거주지(비서울) 필터링 후: {len(matched)}개 (제외: {before_count - len(matched)}개)")
    ...
    if user_info.get('employment_status'):
        ...
        print(f"7. 고용상태({emp_status}) 필터링 후: {len(matched)}개 (제외: {before_count - len(matched)}개)")
    
    # 5. 관련 카테고리 도출
    relevant_categories = _get_relevant_categories(user_info)
    print(f"8. 관련 카테고리: {relevant_categories}")
    ...
    print(f"9. 최소 점수(30점) 필터링 후: {len(matched)}개 (제외: {before_count - len(matched)}개)")
    ...
    for idx, row in matched.head(5).iterrows():
        print(f"   - {row['program_name'][:40]:40} | 점수: {row['priority']:3d} | {row['category_primary']}")
    ...
    print(f"\n✅ 최종 선택된 프로그램: {len(final_results)}개")
    print(f"   카테고리별 분포: {categories_selected}")
    print(f"{'='*60}\n")
